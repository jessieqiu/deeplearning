...
def activation(activation_index, z, alp=1):  #alpha is the constant for leaky relu or ELU
    ...
    activation_set = ["sigmoid","tanh","ReLU","Leaky ReLU","ELU","Maxout"]
...
def maxout_test(z):   #not tested yet  
    z_copy = z.copy()
    sorted_z =  np.sort(z_copy, axis=0)
    print(sorted_z)
    k = 2
    r = sorted_z.shape[0]/k
    print(r)
    thresh = sorted_z[r]
    tem = np.tile(thresh, (sorted_z.shape[0], 1))
    print(tem.shape)
    mask = np.where(z_copy >= tem,1,0)
    print(mask)
...
def forward_all_layers(X_data,weights_1, bias_1, weights_2,bias_2, weights_3,bias_3,activations,alpha): # layers, what else ?  
    ...
    activation_set = ["sigmoid","tanh","ReLU","Leaky ReLU","ELU","Maxout"] #change to 0 1 2 3 4 later,  faster?
   
    #first layer
    
    z1,a1,mask1 = forward_one_layer(weights_1,X_data,bias_1,activations[0],alpha[0])
    
    D1 = np.random.rand(a1.shape[0],a1.shape[1])
    D1 = np.where(D1<keep_prob_1,1,0)
    a1 = np.multiply(D1,a1)
    a1 = a1/keep_prob_1
    
    #second layer
   
    z2,a2,mask2 = forward_one_layer(weights_2,a1,bias_2,activations[1],alpha[1])
    
    D2 = np.random.rand(a2.shape[0],a2.shape[1])
    D2 = np.where(D2<keep_prob_2,1,0)
    ...
    z3,a3,_ = forward_one_layer(weights_3,a2,bias_3,activations[2],alpha[2])
        
        
    return z1,a1,z2,a2,z3,a3,D1,D2,mask1,mask2
...
def getGradient(a1, z2, a2, previous_dldz, w, activation_function,alp,mymask,maxout,keep_prob=0.5):
    dzda = w 
    ...
    dadz = np.multiply(mymask,dadz)
    dadz = dadz/keep_prob
    
    dzdw = np.transpose(a1) 
...
def lastLayerGradient(a, X, i=0, j=y_one_hot.shape[1]):  #i j can be used for mini batch, X is the input to last layer
    if(a.shape==y_one_hot[:,i:j].shape):
        ...
        m = j-i
        dldw = np.dot(dldz,np.transpose(X))/(m) # m or -m?  #do i need to modify x here? 
...
def initialization(neurons,functions):
    ...
    for i in range(len(functions)):
        ...
        if (functions[i]== 0):
            ...
            c[i] = 0.1 #try different value here
        else:
            c[i] = 2    
    print 'sigma,', c
...
def get_test_result(X_data,weights_1, bias_1, weights_2,bias_2, weights_3,bias_3,activations,alpha): # layers, what else ?  
       
    z1,a1,_ = forward_one_layer(weights_1,X_data,bias_1,activations[0],alpha[0])
        
    z2,a2,_ = forward_one_layer(weights_2,a1,bias_2,activations[1],alpha[1])
...
activation_set = ["sigmoid","tanh","ReLU","Leaky ReLU","ELU","Maxout"]
neurons = [500,500,10]
activations = [5,5,0]       #last layer must be 0 to be able to use cross entropy cost function
alphas = [0.1,0.1,1]      #must be corresponding to activation function
...
for i in range(max_iterations):
    ...
    # X is training data
    z1,a1,z2,a2,_,a3,D1,D2,mask1,mask2 = forward_all_layers(np.transpose(X),weights_1, bias_1, weights_2,bias_2,weights_3,bias_3, activations,alphas)
    ...
    #getGradient(a1, z2, a2, previous_dldz, w, activation_function,alp,mask,maxout,keep_prob=0.5):
        
    dldz_2, dldw_2, dldb_2 = getGradient(a1, z2, a2, dldz_3, weights_3, activations[-2],alphas[-2], D2, mask2,keep_prob_2) 

    dldz_1, dldw_1, dldb_1 = getGradient(X.T, z1, a1, dldz_2, weights_2, activations[-3],alphas[-3], D1, mask1, keep_prob_1) 
    ...
    if(i > 0 and i % 100 == 0 ):
        a3_test = get_test_result(np.transpose(X_test),weights_1, bias_1, weights_2,bias_2,weights_3,bias_3, activations,alphas)
...
print 'y optimized test data accuracy', best_test_accuracy
...
#how to test with the dropout here???
a3_final = get_test_result(np.transpose(X_test),best1, best2,best3,best4,best5,best6,best7,best8)
