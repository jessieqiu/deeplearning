...
def getGradient(a1, z2, a2, previous_dldz, w, activation_function,alp,maxout,k):
    dzda = w 
...
def initialization(neurons,functions):
    ...
    for i in range(len(functions)):
        ...
        if (functions[i]== 0):
            ...
            c[i] = 1 #try different value here
        else:
            c[i] = 2    
    print 'sigma,', c
...
alphas = [1,0.1,1]      #must be corresponding to activation function
...
for i in range(max_iterations):
    ...
    # X is training data
    z1,a1,z2,a2,_,a3,mask1,mask2 = forward_all_layers(np.transpose(X),weights_1, bias_1, weights_2,bias_2,weights_3,bias_3, activations,alphas)
    ...
    dldz_2, dldw_2, dldb_2 = getGradient(a1, z2, a2, dldz_3, weights_3, activations[-2],alphas[-2],mask2,k) #could be wrong

    dldz_1, dldw_1, dldb_1 = getGradient(X.T, z1, a1, dldz_2, weights_2, activations[-3],alphas[-3],mask1,k) #could be wrong
    ...
    if(i > 0 and i % 100 == 0  ):
        #_,_,_,_,_,a3_test,_,_ = forward_all_layers(np.transpose(X_test),weights_1, bias_1, weights_2,bias_2,weights_3,bias_3, activations,alphas)
...
#_,_,_,_,_,a3_final,_,_ = forward_all_layers(np.transpose(X_test),best1, best2,best3,best4,best5,best6,best7,best8)
